...
#include "linear.h"
#include "newton.h"
//#include "kernel.h" /* included in "linear.h" */
...
static int solve_l2r_l1l2_svc_kernel(const problem *prob, const parameter *param, double *w, double *alpha, double Cp, double Cn, int max_iter=DEFAULT_MAX_ITER, const kernel_parameter *kernel_param=0)
{
	int l = prob->l;
	int w_size = prob->n;
	...
	SVC_Q *svc_q = 0;
	if(kernel_param){
		svc_q = new SVC_Q(*prob,*kernel_param,y);
	}
	...
	while (iter < max_iter)
	{
		...
		for (s=0; s<active_size; s++)
		{
			...
			// TODO: from KKT condition, yi * (xi . w) = Qi . alpha
			// (q{ij} := yi yj (xi . xj); (xi . xj) is assumed to be precomputed if kernelized)
			if(!kernel_param){
...
static int solve_l2r_l1l2_svc(const problem *prob, const parameter *param, double *w, double Cp, double Cn, int max_iter=DEFAULT_MAX_ITER)
{
	double *alpha = new double[prob->l];
	int iter = solve_l2r_l1l2_svc_kernel(prob, param, w, alpha, Cp, Cn, max_iter);
	delete [] alpha;
	return iter;
}
...
static int solve_l2r_l1l2_svr(const problem *prob, const parameter *param, double *w, int max_iter=DEFAULT_MAX_ITER)
{
...
static int solve_l2r_lr_dual(const problem *prob, const parameter *param, double *w, double Cp, double Cn, int max_iter=DEFAULT_MAX_ITER)
{
	int l = prob->l;
	int w_size = prob->n;
	double eps = param->eps;
...
static void train_one(const problem *prob, const parameter *param, const kernel_parameter *kernel_param, double *w, double Cp, double Cn)
{
	int solver_type = param->solver_type;
	int dual_solver_max_iter = DEFAULT_MAX_ITER;
	int iter;
	...
	switch(solver_type)
	{
		...
		{
			if(kernel_param){
				iter = solve_l2r_l1l2_svc_kernel(prob, param, 0, w, Cp, Cn, dual_solver_max_iter, kernel_param);
...
static void find_parameter_C(const problem *prob, parameter *param_tmp, const kernel_parameter *kernel_param, double start_C, double max_C, double *best_C, double *best_score, const int *fold_start, const int *perm, const problem *subprob, int nr_fold)
{
	// variables for CV
	int i;
	...
	*best_C = start_C;
	...
	while(param_tmp->C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *submodel = train(&subprob[i],param_tmp,kernel_param);
...
// Interface functions
//
model* train(const problem *prob, const parameter *param, const kernel_parameter *kernel_param)
{
	problem newprob;
	...
	if(kernel_param){
		model_->kernelized = 1;
		model_->kernel_param = *kernel_param;
		model_->nr_feature=l;
	}else{
		model_->kernelized = 0;
		if(prob->bias>=0)
	...
	if(check_regression_model(model_))
	{
		...
		model_->nr_class = 2;
		model_->label = NULL;
		train_one(prob, param, kernel_param, model_->w, 0, 0);
	}
	else if(check_oneclass_model(model_))
	{
		if(kernel_param){
			fprintf(stderr,"Error: The use of kernel function is not implemented for one-class SVM.\n");
			return 0;
		}
		model_->w = Malloc(double, w_size);
		model_->nr_class = 2;
	...
	else
	{
		...
		{
			if(kernel_param){
				fprintf(stderr,"Error: The use of kernel function is not implemented for Crammer-Singer SVM.\n");
				return 0;
			}
		...
		else
		{
			if(nr_class == 2)
			{
				...
				train_one(&sub_prob, param, kernel_param, model_->w, weighted_C[0], weighted_C[1]);
			}
			else
			{
				...
				for(i=0;i<nr_class;i++)
				{
					...
					train_one(&sub_prob, param, kernel_param, w, weighted_C[i], param->C);
		...
		// Restore original order of the dataset if needed
		if(kernel_param){
			// Reorder model_->w by the original order
			double *w_reorder = Malloc(double, w_size);
			for(int ii = 0; ii < w_size; ++ii){
				w_reorder[perm[ii]] = model_->w[ii];
			}
			free(model_->w);
			model_->w = w_reorder;
		}
...
void cross_validation(const problem *prob, const parameter *param, const kernel_parameter *kernel_param, int nr_fold, double *target)
{
	int i;
	int *fold_start;
	int l = prob->l;
	...
	for(i=0;i<nr_fold;i++)
	{
		...
		struct model *submodel = train(&subprob,param,kernel_param);
		for(j=begin;j<end;j++)
			target[perm[j]] = predict(submodel,prob->x[perm[j]]);
		free_and_destroy_model(&submodel);
		free(subprob.x);
...
void find_parameters(const problem *prob, const parameter *param, const kernel_parameter *kernel_param, int nr_fold, double start_C, double start_p, double *best_C, double *best_p, double *best_score)
{
	// prepare CV folds
	...
	*best_p = -1;
	if(param->solver_type == L2R_LR || param->solver_type == L2R_L2LOSS_SVC)
	{
		...
		find_parameter_C(prob, &param_tmp, kernel_param, start_C, max_C, &best_C_tmp, &best_score_tmp, fold_start, perm, subprob, nr_fold);
	...
	else if(param->solver_type == L2R_L2LOSS_SVR)
	{
		...
		*best_score = INF;
		...
		for(; i >= 0; i--)
		{
			...
			find_parameter_C(prob, &param_tmp, kernel_param, start_C_tmp, max_C, &best_C_tmp, &best_score_tmp, fold_start, perm, subprob, nr_fold);
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	int idx;
	int n;
	if(!model_->kernelized && model_->bias>=0)
	...
	if(model_->kernelized){
		// Computation of the predicted value depends on the problem setup
		// since the KKT condition (the computation using dual variables)
		// may differ.
		if(model_->param.solver_type == L2R_L2LOSS_SVC_DUAL
		   || model_->param.solver_type == L2R_L1LOSS_SVC_DUAL){
			// In this case, for a test sample X, its predicted value Y_C
			// for the C-th class is computed as
			// Y_C = sum_{i=1}^l y_i alpha_{i,C} K(x_i, X),
			// where (x_i, y_i)_{i=1}^l are training samples
			// and K is the kernel function.

			double *kvalue = Malloc(double,n);
#ifdef _OPENMP
#pragma omp parallel for private(i) schedule(guided)
#endif
			for(i=0;i<n;i++){
				kvalue[i] = Kernel::k_function(x,model_->SV[i],model_->kernel_param);
			}

			for(i=0;i<nr_w;i++)
				for(j=0;j<n;j++)
					dec_values[i] += model_->y[j] * model_->w[j*nr_w+i] * kvalue[j];
		}else{
			fprintf(stderr, "ERROR: The model parameter obtained by the solver [%d] cannot be combined with a kernel [%d]\n", model_->param.solver_type, model_->kernel_param.kernel_type);
			exit(-1);
		}
	}else{
...
int save_model(const char *model_file_name, const struct model *model_, const struct problem * prob)
{
	int i;
	...
	if(model_->kernelized && (model_->kernel_param.kernel_type != PRECOMPUTED) && prob == 0){
		fprintf(stderr, "ERROR: For kernelized model (except PRECOMPUTED), parameter \"prob\" must be specified\n");
		return -1;
	}
	...
	// About kernel
	if(model_->kernelized){
		fprintf(fp,"nr_feat_orig %d\n", prob->n - (model_->bias >= 0 ? 1 : 0));
		fprintf(fp,"kernel_type %s\n", kernel_type_table[model_->kernel_param.kernel_type]);
		if(model_->kernel_param.kernel_type == POLY)
			fprintf(fp,"degree %d\n", model_->kernel_param.degree);
		if(model_->kernel_param.kernel_type == POLY || model_->kernel_param.kernel_type == RBF || model_->kernel_param.kernel_type == SIGMOID)
			fprintf(fp,"gamma %.17g\n", model_->kernel_param.gamma);
		if(model_->kernel_param.kernel_type == POLY || model_->kernel_param.kernel_type == SIGMOID)
			fprintf(fp,"coef0 %.17g\n", model_->kernel_param.coef0);
	}
	...
	if(model_->kernelized){
		...
		for(i=0; i<w_size; i++)
		{
			...
			fprintf(fp,"y:%.g ",prob->y[i]);
			if(model_->kernel_param.kernel_type == PRECOMPUTED){
				fprintf(fp,"0:%d ",i+1);
			}else{
				const feature_node *p = prob->x[i];
				while(p->index != -1)
				{
...
{\
	...
	{\
		fprintf(stderr, "ERROR: fscanf failed to read the model (format = %s, var = %s)\n", _format, #_var);\
		EXIT_LOAD_MODEL()\
	}\
}while(0)
...
class ReadlineCF{
	...
	char * line;
public:
	ReadlineCF() : max_line_len(1024), line(Malloc(char,max_line_len)){}

	~ReadlineCF(){
		free(line);
	}

	char * operator()(FILE * input){
		int len;
...
int only_spaces(const char * buf){
	while(*buf != '\n' || *buf == '\0'){
		if(strchr("\t\n\v\f\r ", *buf) == NULL) return 0;
	}
	return 1;
}
...
struct model *load_model(const char *model_file_name)
{
	...
	int i;
	int nr_feature, nr_feature_orig;
	int n;
	int nr_class;
	double bias;
	double rho;
	...
	model_->label = NULL;
	model_->nr_feature_orig = 0;
	model_->kernelized = 0;
	model_->kernel_param.kernel_type = -1;
	model_->kernel_param.degree = -1;
	model_->kernel_param.gamma = -1.0;
	model_->kernel_param.coef0 = 0.0;
	...
	while(1)
	{
		...
		else if(strcmp(cmd,"nr_feat_orig")==0)
		{
			FSCANF(fp,"%d",&nr_feature_orig);
			model_->nr_feature_orig = nr_feature_orig;
		}
		...
		else if(strcmp(cmd,"kernel_type")==0)
		{
			...
			for(i=0;kernel_type_table[i];i++)
			{
				...
				{
					model_->kernel_param.kernel_type=i;
					model_->kernelized=1;
					break;
				}
			}
		...
		else if(strcmp(cmd,"degree")==0)
		{
			int degree;
			FSCANF(fp,"%d",&degree);
			model_->kernel_param.degree=degree;
			model_->kernelized=1;
		}
		else if(strcmp(cmd,"gamma")==0)
		{
			double gamma;
			FSCANF(fp,"%lf",&gamma);
			model_->kernel_param.gamma=gamma;
			model_->kernelized=1;
		}
		else if(strcmp(cmd,"coef0")==0)
		{
			double coef0;
			FSCANF(fp,"%lf",&coef0);
			model_->kernel_param.coef0=coef0;
			model_->kernelized=1;
		}
		else
		{
	...
	if(strcmp(cmd,"w")==0 && model_->kernelized){
		fprintf(stderr,"Specification \"w\" cannot be used in kernelized model\n");
		EXIT_LOAD_MODEL()
	}
	if(strcmp(cmd,"SV")==0 && !(model_->kernelized)){
		fprintf(stderr,"Specification \"SV\" cannot be used in non-kernelized model\n");
		EXIT_LOAD_MODEL()
	}
	if(model_->nr_feature_orig == 0){
		model_->nr_feature_orig = model_->nr_feature;
	}

	// Check kernel parameters
	if(model_->kernelized){
		if(model_->kernel_param.kernel_type < 0){
			fprintf(stderr,"\"kernel_type\" must be specified\n");
			EXIT_LOAD_MODEL()
		}

		if((model_->kernel_param.kernel_type == POLY || model_->kernel_param.kernel_type == RBF || model_->kernel_param.kernel_type == SIGMOID) &&
		model_->kernel_param.gamma < 0){
			fprintf(stderr,"\"gamma\" must be nonnegative\n");
			EXIT_LOAD_MODEL()
		}

		if(model_->kernel_param.kernel_type == POLY && model_->kernel_param.degree < 0){
	...
	if(model_->kernelized){
		// partly brought from svm.cpp in LIBSVM
		...
		elements += model_->nr_feature;

		fseek(fp,pos,SEEK_SET);

		int l = model_->nr_feature;
		...
		for(i=0;i<l;i++)
		{
			...
			char * nline = readline.get();
			if(only_spaces(nline)){
				--i;
				continue;
			}
			p = strtok(nline, " \t");
			model_->w[i*nr_w+0] = strtod(p,&endptr);
			for(int k=1;k<nr_w;k++)
			{
				p = strtok(NULL, " \t");
				model_->w[i*nr_w+k] = strtod(p,&endptr);
			}
			...
			while(1)
			{
				...
				double val_num = strtod(val,&endptr);
				if(*endptr != '\0'){
					fprintf(stderr,"Invalid value found: %s\n",val);
					EXIT_LOAD_MODEL()
				}
				if(strcmp(idx, "y") == 0){
					model_->y[i] = val_num;
					...
					int idx_num = (int) strtol(idx,&endptr,10);
					if(*endptr != '\0'){
						fprintf(stderr,"Invalid index found: %s\n",val);
						EXIT_LOAD_MODEL()
					}
					x_space[j].index = idx_num;
					x_space[j].value = val_num;
					++j;
				}
			}
			if(y_read != 1){
				fprintf(stderr,"Label value not included or duplicated in the instance #%d (%d found)\n", i+1, y_read);
				EXIT_LOAD_MODEL()
			}
...
const char *check_parameter(const problem *prob, const parameter *param, const kernel_parameter *kernel_param)
{
	if(param->eps <= 0)
		return "eps <= 0";
	...
	if (param->solver_type == ONECLASS_SVM)
	{
		for(int i=0;i<prob->l;i++)
			if (prob->W[i] != 1)
				return "-W not supported for one-class SVM\n";
	}
	...
	if(kernel_param){
		if((kernel_param->kernel_type != LINEAR) &&
			(param->solver_type != L2R_L2LOSS_SVC_DUAL
			&& param->solver_type != L2R_L1LOSS_SVC_DUAL))
			return "Kernel function cannot be selected for the solver. For the solver, please do not specify the option \"-t\".\n";

		if((kernel_param->kernel_type == POLY || kernel_param->kernel_type == RBF || kernel_param->kernel_type == SIGMOID) &&
		kernel_param->gamma < 0)
			return "gamma < 0";

		if(kernel_param->kernel_type == POLY && kernel_param->degree < 0)
